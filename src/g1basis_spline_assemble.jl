...
for i in 1:nbe(hm) #I go through all the half edges
    if opp(hm,i) in list
        ...
        if opp(hm,i)!=0 && is_singular_edge(hm,i) #inner singular hedge
            ...
            #g1basiscp=nonzeroctrpts;#g1basis_edges_kn(knts,valances);
            #println(knts)
            g1basiscp=g1basis_edges_kn(knts,valances);
            col=size(g1basiscp,2);
            #println(col)
            for k in 1:col
                data=findnz(g1basiscp[:,k]);
                ...
                for t in 1:length(row)
                    ...
                    #if idx(gsbas,i,w,h)>=1 && idx(gsbas,next(hm,opp(hm,i)),w,h)>=1
                        if j==2
                            #=println(idx(gsbas,i,w,h));
                            if idx(gsbas,i,w,h)<1
                                sleep(10)
                            end=#
                            push!(bigrow,idx(gsbas,i,w,h));
                            push!(bigcol,globcol);
